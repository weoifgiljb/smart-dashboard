...
- 2025-12-06
  - AI 对话系统重构（会话管理、流式响应支持、历史记录持久化）
  - UI/UX 全面重构（Bento Grid、沉浸式番茄钟、统一设计规范）
  - Login/Register 页面现代化改造
  - AI 问答与日历打卡功能修复（Regex/Icon 兼容性）
  - CI/CD 修复（前端构建通过，GitHub Pages 部署配置）
- 2025-11-24
  - 仪表盘交互增强（图表下钻、日期筛选）
  - 日记编辑器升级（Markdown/Monaco Editor）
  - 书籍搜索 Bug 修复与 Maven 构建修复
- 2025-02-26
  - 工程化规范落地（ESLint + Prettier + Husky + Lint-staged）
  - 单元测试框架搭建（Vitest + Vue Test Utils + Element Plus 支持）
  - 性能优化（虚拟滚动 VirtualList 组件实现与集成，构建产物分析配置）
- 2025-11-21
...
- **AI 对话系统重构 (AI Conversation System Overhaul)**
  - **会话管理**: 引入 `Conversation` 模型，支持创建多组独立对话，侧边栏实时显示历史会话列表。
  - **流式响应**: 实现了 `Flux<String>` 后端流式接口 (`/ai/chat/stream`)，前端打字机效果流畅展示 AI 回复。
  - **持久化修复**: 彻底修复了聊天记录 (`Chat`) 未正确保存到 MongoDB 的问题，现在所有对话内容（包括 User/AI）均完整入库。
  - **异常处理**: 增强了前端的容错机制，当流式接口失败时自动回退到普通 REST 接口。

- **UI/UX 全面重构 (UI/UX Overhaul)**
  - **全局设计系统 (Global Design System)**: 在 `tokens.css` 中建立了统一的设计规范，采用 Emerald (主色) + Blue (辅色) 的活力配色方案，统一了阴影、圆角和排版风格。
  - **仪表盘 (Dashboard)**: 采用 **Bento Grid** 布局重构 KPI 卡片与图表区，新增欢迎区与快捷操作，提升了信息密度与视觉层级。
  - **日历 (Calendar)**: 重新设计了年度热力图（GitHub 风格），增加了每日详情侧边抽屉，优化了打卡按钮状态反馈。
  - **番茄钟 (Pomodoro)**: 实现了沉浸式 UI，采用渐变进度环 (`progress-container`) 和悬浮控制按钮，根据工作/休息模式自动切换主题氛围。
  - **任务 (Tasks)**: 优化了表格、看板、甘特图和统计视图的视觉样式，引入了更现代的过滤器栏和优先级标签。
  - **单词 (Words)**: 单词复习界面重构为卡片翻转模式，增强了拼写练习的交互体验；新增了学习趋势图表。
  - **认证页面 (Login/Register)**: 现代化改造，采用左右分栏布局（桌面端），左侧展示动态品牌视觉，右侧为极简表单。

- **问题修复与技术改进 (Bug Fixes & Technical Improvements)**
  - **AI 问答 (AI Chat)**:
    - **正则兼容性修复**: 修复了 Safari/旧版浏览器中 `SyntaxError: Invalid regular expression` 错误。将不兼容的 Unicode Property Escapes (`\u{1F300}-\u{1F6FF}`) 替换为具体的 Unicode 范围 (`\uD83C-\uD83E` 等)，解决了 Emoji 匹配导致的页面崩溃问题。
  - **日历 (Calendar)**:
    - **图标引用修复**: 修复了因引用不存在的 `CalendarCheck` 图标导致的 `SyntaxError`，替换为标准的 `Calendar` 图标。
    - **打卡逻辑优化**: 解耦了“打卡动作”与“数据刷新”的强依赖，确保即使统计数据刷新失败，用户也能收到打卡成功的正向反馈。
  - **CI/CD 修复**:
    - **前端构建修复**: 修复了 `vue-tsc` 检查出的多个未使用的变量和图标引用错误（AIChat, Login, Register, Tasks, VocabularyReview, Words），确保 CI 构建流程顺利通过。
    - **GitHub Pages 适配**: 配置了 `.github/workflows/pages.yml`，设置了 `base` 路径与路由 `history` 模式，支持 GitHub Pages 子路径部署。
...
- **工程化规范 (Engineering Standards)**
  - **Linting & Formatting**: 全面引入 ESLint 和 Prettier，配置了严格的代码风格规则。
  - **Git Hooks**: 配置 Husky 和 lint-staged，确保提交前自动执行代码检查和格式化。
  - **代码修复**: 批量修复了项目中大量的 ESLint 报错（包括未使用的变量、类型断言、组件命名规范等）。

- **单元测试 (Unit Testing)**
  - **框架搭建**: 集成 Vitest 作为单元测试运行器，配置 `jsdom` 环境。
  - **组件测试**: 编写了 `AppButton.spec.ts` 示例测试，验证了 Element Plus 组件在测试环境下的正确渲染。
  - **配置优化**: 解决了 Element Plus 组件在测试中无法解析的问题，配置了全局存根。

- **性能优化 (Performance Optimization)**
  - **虚拟滚动**: 实现了通用的 `VirtualList.vue` 组件，用于优化长列表渲染性能。
  - **集成应用**: 在单词列表页 (`Words.vue`) 中集成虚拟滚动，当数据量超过 50 条时自动启用，显著减少 DOM 节点数量。
  - **构建分析**: 在 `vite.config.ts` 中配置了构建产物大小预警（chunkSizeWarningLimit），并尝试引入可视化分析插件（目前因依赖问题暂时注释）。
  - **构建修复**: 解决了 `offlineQueue.ts` 中 TypeScript 编译与 ESLint 规则的冲突，确保 `npm run build` 顺利通过。

- **CI/CD 流程 (CI/CD Pipeline)**
  - **GitHub Actions**: 创建了 `.github/workflows/ci.yml` 工作流文件。
  - **自动化任务**:
    - **Frontend**: 包含安装依赖、Lint 检查、单元测试、构建打包。
    - **Backend**: 包含 JDK 17 环境设置、Maven 构建。
  - **触发机制**: 配置为在 `master`/`main` 分支推送或 Pull Request 时自动触发。
  - **持续部署 (CD)**:
    - **Artifacts**: 配置了前端 `dist` 和后端 `jar` 包的构建产物上传。
    - **Deploy Job**: 新增部署任务，依赖于构建任务成功。
    - **SSH/SCP**: 集成 `appleboy/ssh-action` 和 `scp-action` 实现自动化部署到 Linux 服务器。
    - **部署流程**: 自动备份旧版本 -> 上传新文件 -> 替换静态资源与 Jar 包 -> (预留) 重启服务。
